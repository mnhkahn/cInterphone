...
public class CInterphoneEngine implements RegisterAgentListener {
	...
	UserAgentProfile getUserAgentProfile(String suffix) {
		...
		System.out.println(PreferenceManager.getDefaultSharedPreferences(
				getUIContext()).getString(Settings.PREF_USERNAME,
				Settings.DEFAULT_USERNAME) + PreferenceManager.getDefaultSharedPreferences(
	...
	public boolean StartEngine() {
		...
		for (int i = 1; i < LINES; i++)
			user_profiles[1] = getUserAgentProfile("");
...
//										: "udp");
				...
				String version = "Sipdroid/" + CInterphone.getVersion() + "/"
						+ Build.MODEL;
	...
	void setOutboundProxy(SipProvider sip_provider, int i) {
		try {
			String host = PreferenceManager
					.getDefaultSharedPreferences(getUIContext())
					.getString(Settings.PREF_PROTOCOL + (i != 0 ? i : ""),
							Settings.DEFAULT_PROTOCOL).equals("tls") ? PreferenceManager
					.getDefaultSharedPreferences(getUIContext()).getString(
							Settings.PREF_SERVER + (i != 0 ? i : ""),
							Settings.DEFAULT_SERVER) : null;
			if (host != null && host.equals(Settings.DEFAULT_SERVER))
				host = "www1.pbxes.com";
			if (sip_provider != null)
				sip_provider.setOutboundProxy(
						new SocketAddress(IpAddress.getByName(PreferenceManager
								.getDefaultSharedPreferences(getUIContext())
	...
	public boolean call(String target_url, boolean force) {
		...
		System.out.println(ua.user_profile.contact_url);
		if (!ua.user_profile.audio && !ua.user_profile.video) {
			ua.printLog("ONLY SIGNALING, NO MEDIA");
		}
