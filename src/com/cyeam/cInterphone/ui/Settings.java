...
public class Settings extends PreferenceActivity implements
	...
	public static final String DEFAULT_SERVER = "121.199.55.106";
				...
				if (key.equals(PREF_PROTOCOL + j)) {
					if (sharedPreferences.getString(PREF_SERVER + j,
						...
						lp.setValue(sharedPreferences.getString(
								PREF_PROTOCOL + j, DEFAULT_PROTOCOL).equals(
								"tls") ? "5070" : "5061");
					} else {
	...
	public void updateSummaries() {
		...
		String server = settings.getString(PREF_SERVER, DEFAULT_SERVER);
		
		getPreferenceScreen().findPreference(PREF_USERNAME)
				.setSummary(username);
		...
		if (settings.getString(PREF_DOMAIN, DEFAULT_DOMAIN).length() == 0) {
			getPreferenceScreen().findPreference(PREF_DOMAIN).setSummary(
					getString(R.string.settings_domain2));
		} else {
			getPreferenceScreen().findPreference(PREF_DOMAIN).setSummary(
					settings.getString(PREF_DOMAIN, DEFAULT_DOMAIN));
		}
		if (settings.getString(PREF_FROMUSER, DEFAULT_FROMUSER).length() == 0) {
			getPreferenceScreen().findPreference(PREF_FROMUSER).setSummary(
					getString(R.string.settings_callerid2));
		} else {
			getPreferenceScreen().findPreference(PREF_FROMUSER).setSummary(
					settings.getString(PREF_FROMUSER, DEFAULT_FROMUSER));
		}
		getPreferenceScreen().findPreference(PREF_PORT).setSummary(
				settings.getString(PREF_PORT, DEFAULT_PORT));
		...
		// if (!settings.getString(PREF_PREF,
		// DEFAULT_PREF).equals(VAL_PREF_PSTN)) {
