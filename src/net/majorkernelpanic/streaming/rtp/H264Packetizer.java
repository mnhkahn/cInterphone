...
public class H264Packetizer extends AbstractPacketizer implements Runnable {
	...
	private void send() throws IOException, InterruptedException {
		...
		if (streamType == 0) {
			...
			// Log.e("FUCK", "streamType==0" + " " + naluLength);
			if (naluLength > 100000 || naluLength < 0)
				resync();
		} else if (streamType == 1) {
			Log.e("FUCK", "streamType==1");
		...
		if (naluLength > MAXPACKETSIZE - rtphl - 2) {
			Log.e("ERROR", "too large size" + naluLength);
		}
//		if (naluLength <= MAXPACKETSIZE - rtphl - 2) {
			// Log.e("FUCK", "Single");
			buffer = socket.requestBuffer();
			...
			// 在放入naluLength-1的内容，一共naluLength个字节
...
//		else {
////			 Log.e("FUCK", "" + naluLength);
//			// Set FU-A header
//			// 取后5位，SER设为100，头包
//			header[1] = (byte) (header[4] & 0x1F); // FU header type
//			header[1] += 0x80; // Start bit
//			// Set FU-A indicator
//			// 取header[0]2、3位，TYPE设为28
