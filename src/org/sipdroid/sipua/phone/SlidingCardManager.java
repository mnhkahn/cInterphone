 ...
 */
public class SlidingCardManager implements
	...
	/**
	 * Reference to the InCallScreen activity that owns us. This will be null if
	...
	private int mTouchDownY; // Y-coordinate of the DOWN event that started the
								// slide
	 ...
	 * 
	 * @param phone
	 *            the Phone app's Phone instance
	 * @param inCallScreen
	 *            the InCallScreen activity
	 * @param mainFrame
	 *            the InCallScreen's main frame containing the in-call UI
	 *            elements
	 */
	/* package */public void init(Phone phone, InCallScreen inCallScreen,
			ViewGroup mainFrame) {
		if (DBG)
			log("init()...");
		mPhone = phone;
		...
		mSlideDownHint = (TextView) mInCallScreen
				.findViewById(R.id.slideDownHint);
	...
	/**
	 * Null out our reference to the InCallScreen activity. This indicates that
	 * the InCallScreen activity has been destroyed.
	 */
	...
	/**
	 * Update the "permanent" position (ie. the actual layout params) of the
	 * sliding card based on the current call state.
	 * 
	 * This method sets mCardAtTop, mCallEndedState, mCardPreferredX, and
	 * mCardPreferredY. It also repositions the PopupWindow if it's showing.
	 * 
	 * Note that *while sliding* we manually reposition the card on every motion
	 * event that comes in. The x/y position we set here determines where the
	 * card should be while *not* sliding.
	 * 
	 * TODO: If the card position changes for some reason *other* than user
	 * action (i.e. as a result of an onPhoneStateChanged() callback), we should
	 * smoothly animate the position change! (For example, if you're in a call
	 * and the other end hangs up, the card should switch to "Call ended" mode
	 * and smoothly animate to the bottom position.)
	 */
	public void updateCardPreferredPosition() {
		if (DBG)
			log("updateCardPreferredPosition()...");
		// if (DBG) log("- card's LayoutParams: " +
		// mCallCard.getLayoutParams());
		...
		if (mMainFrame.getWindowToken() == null) {
			if (DBG)
				log("updateCardPreferredPosition: View hierarchy unattached; bailing...");
			return;
		}

		/*
		 * if (mMainFrame.getHeight() == 0) { // The code here needs to know the
		 * sizes and positions of some // views in the mMainFrame view
		 * hierarchy, so you're only // allowed to call this method *after* the
		 * whole in-call UI // has been measured and laid out. // (This is why
		 * we defer calling showPopup() until an // onGlobalLayout() call comes
		 * in.) throw new IllegalStateException(
		...
		if (height == 0) {
			...
			RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) mSlideUp
					.getLayoutParams();
		...
		final int popupBottomPosY = mainFrameY + mMainFrame.getHeight()
				- height;

		if (Receiver.ccCall != null
				&& Receiver.ccCall.getState() != Call.State.DISCONNECTED) {
		...
		if (DBG)
			log("==> Setting card preferred position (mCardAtTop = "
					+ mCardAtTop + ") to: " + mCardPreferredX + ", "
					+ mCardPreferredY);
	...
	public/* package */void updateCardSlideHints() {
		...
		final boolean hasRingingCall = Receiver.ccCall != null
				&& Receiver.ccCall.getState() == Call.State.INCOMING;
	...
	/**
	 * Sets the text of the "slide hints" based on the specified resource IDs.
	 * (A resource ID of zero means "hide the hint and arrow".)
	 */
	private void setSlideHints(int upHintResId, int downHintResId) {
		...
		if (DBG) {
			String upHint = (upHintResId != 0) ? mInCallScreen
					.getString(upHintResId) : "<empty>";
			String downHint = (downHintResId != 0) ? mInCallScreen
					.getString(downHintResId) : "<empty>";
			log("setSlideHints: UP '" + upHint + "', DOWN '" + downHint + "'");
		}

		mSlideUp.setVisibility((upHintResId != 0) ? View.VISIBLE : View.GONE);
		if (upHintResId != 0)
			mSlideUpHint.setText(upHintResId);

		mSlideDown.setVisibility((downHintResId != 0) ? View.VISIBLE
				: View.GONE);
		if (downHintResId != 0)
			mSlideDownHint.setText(downHintResId);
	}
	...
	/* package */void handleCallCardTouchEvent(MotionEvent ev) {
		...
		if (mInCallScreen == null || mInCallScreen.isFinishing()) {
			Log.i(LOG_TAG,
					"handleCallCardTouchEvent: InCallScreen gone; ignoring touch...");
			return;
		}
		...
		if (isSlideInProgress()) {
			...
			if (now - mTouchDownTime > 1000 || InCallScreen.pactive
				...
				case MotionEvent.ACTION_CANCEL:
					...
					if (DBG)
						log("handleCallCardTouchEvent: ACTION_CANCEL: " + ev);
					abortSlide();
					break;
				}
		} else {
	...
	/**
	 * Start sliding the card. Called when we get a DOWN touch event on the
	...
	/* package */void startSliding(int x, int y) {
		...
		if (mCallEndedState) {
			if (DBG)
				log("startSliding: CALL ENDED state; ignoring...");
			return;
		}
	...
	/* package */void updateWhileSliding(int y) {
		...
		// TODO: this block is duplicated above; use a single helper method
		// instead.
		mMainFrame.getLocationInWindow(mTempLocation);
		final int mainFrameX = mTempLocation[0];
		final int mainFrameY = 0; // mTempLocation[1];
		...
		if (newCardTop < popupTopPosY)
			newCardTop = popupTopPosY;
		else if (newCardTop > popupBottomPosY)
			newCardTop = popupBottomPosY;
	...
	/**
	 * Stop sliding the card. Called when we get an UP touch event while
	 * sliding.
	 */
	/* package */void stopSliding(int y) {
		int totalSlideAmount = y - mTouchDownY;
		if (DBG)
			log("stopSliding: Total slide delta: " + totalSlideAmount);
		...
		if (!mCardAtTop)
			totalSlideAmount = -totalSlideAmount;

		if (totalSlideAmount >= slideDistanceRequired) {
			if (DBG)
				log("==> Slide was far enough! slid " + totalSlideAmount
						+ ", needed >= " + slideDistanceRequired);
			finishSuccessfulSlide();
		} else {
			if (DBG)
				log("==> Didn't slide enough to take action: slid "
						+ totalSlideAmount + ", needed >= "
						+ slideDistanceRequired);
			abortSlide();
		}
	}

	/**
	 * The user successfully completed a "slide" operation. Activate whatever
	 * action the slide was supposed to trigger.
	 * 
	 * (That could either be (1) hang up the ongoing call(s), or (2) answer an
	 * incoming call.)
	 * 
	 * This method is responsible for triggering any screen updates that need to
	 * happen, based on any internal state changes due to the slide.
	 */
	...
	public static class WindowAttachNotifierView extends View {
		...
		protected void onAttachedToWindow() {
			...
			if (DBG)
				mSlidingCardManager
						.log("WindowAttachNotifierView: onAttachedToWindow!");
