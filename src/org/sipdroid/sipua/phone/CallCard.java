 ...
 */
public class CallCard extends FrameLayout implements
	...
	public CallCard(Context context, AttributeSet attrs) {
		...
		inflater.inflate(R.layout.call_card, // resource
				this, // root
				true);
	...
	protected void onFinishInflate() {
		...
		// Text colors
		mTextColorConnected = getResources().getColor(
				R.color.incall_textConnected);
	...
	void updateState(Phone phone) {
		...
		if (state == Phone.State.RINGING) {
			...
			// *after* a call ends...
			// For now:
			Log.w(LOG_TAG, "CallCard updateState: overall Phone state is "
					+ state);
			updateForegroundCall(phone);
		}
	}
	...
	private void updateForegroundCall(Phone phone) {
		...
		if (fgCall.isIdle() && !fgCall.hasConnections()) {
			...
			// displayMainCallStatus() will notice if the call we passed in is
			// on
	...
	public void displayMainCallStatus(Phone phone, Call call) {
		...
		case ACTIVE:
			...
			callCardBackgroundResid = landscapeMode ? R.drawable.incall_frame_connected_tall_land
		...
		case HOLDING:
			...
			callCardBackgroundResid = landscapeMode ? R.drawable.incall_frame_hold_tall_land
		...
		case DISCONNECTED:
			...
			callCardBackgroundResid = landscapeMode ? R.drawable.incall_frame_ended_tall_land
		...
		case ALERTING:
			...
			callCardBackgroundResid = landscapeMode ? R.drawable.incall_frame_normal_tall_land
		...
		case IDLE:
			...
			Log.w(LOG_TAG,
					"displayMainCallStatus: IDLE call in the main call card!");
			break;

		default:
			Log.w(LOG_TAG, "displayMainCallStatus: unexpected call state: "
					+ state);
			break;
		}
...

		{
			...
			if (conn == null) {
				...
				if (o instanceof PhoneUtils.CallerInfoToken) {
					runQuery = mPhotoTracker
							.isDifferentImageRequest(((PhoneUtils.CallerInfoToken) o).currentInfo);
				} else {
				...
				if (runQuery) {
					...
					PhoneUtils.CallerInfoToken info = PhoneUtils
							.startGetCallerInfo(getContext(), conn, this, call);
					updateDisplayForPerson(info.currentInfo, isPrivateNumber,
							!info.isFinal, call);
				} else {
	...
	public void onQueryComplete(int token, Object cookie, CallerInfo ci) {
		if (DBG)
			log("onQueryComplete: token " + token + ", cookie " + cookie
					+ ", ci " + ci);

		if (cookie instanceof Call) {
			// grab the call object and update the display for an individual
			// call,
			...
			((TextView) cookie).setText(PhoneUtils
					.getCompactNameFromCallerInfo(ci, getContext()));
		}
	}

	/**
	 * Implemented for ContactsAsyncHelper.OnImageLoadCompleteListener
	 * interface. make sure that the call state is reflected after the image is
	 * loaded.
	 */
	...
	/**
	 * Updates the "upper" and "lower" titles based on the current state of this
	 * call.
	 */
	private void updateCardTitleWidgets(Phone phone, Call call) {
		...
		if (DBG)
			log("updateCardTitleWidgets: " + cardTitle);
	...
	/**
	 * Returns the "card title" displayed at the top of a foreground ("active")
	 * CallCard to indicate the current state of this call, like "Dialing" or
	 * "In call" or "On hold". A null return value means that there's no title
	...
	/**
	 * Updates the "on hold" box in the "other call" info area (ie. the stuff in
	 * the otherCallOnHoldInfo block) based on the specified Call. Or, clear out
	 * the "on hold" box if the specified call is null or idle.
	 */
	public void displayOnHoldCallStatus(Phone phone, Call call) {
		...
		case HOLDING:
			...
			// First, see if we need to query.
			{
				...
				PhoneUtils.CallerInfoToken info = PhoneUtils
						.startGetCallerInfo(getContext(), call, this,
								mOtherCallOnHoldName);
				name = PhoneUtils.getCompactNameFromCallerInfo(
						info.currentInfo, getContext());
			}
	...
	/**
	 * Updates the "Ongoing call" box in the "other call" info area (ie. the
	 * stuff in the otherCallOngoingInfo block) based on the specified Call. Or,
	 * clear out the "ongoing call" box if the specified call is null or idle.
	 */
	public void displayOngoingCallStatus(Phone phone, Call call) {
		if (DBG)
			log("displayOngoingCallStatus(call =" + call + ")...");
		if (call == null) {
		...
		case ALERTING:
			...
			// The call here is always "ongoing", so use the green "connected"
			// frame
	...
	/**
	 * Updates the name / photo / number / label fields on the CallCard based on
	 * the specified CallerInfo.
	 * 
	...
	private void updateDisplayForPerson(CallerInfo info,
		...
		mPhotoTracker
				.setPhotoState(ContactsAsyncHelper.ImageTracker.DISPLAY_IMAGE);
		...
		if (info != null) {
			...
			personUri = ContentUris.withAppendedId(People.CONTENT_URI,
					info.person_id);
		} else {
			{
				name = getContext().getString(R.string.unknown);
		...
		if (isTemporary && (info == null || !info.isCachedPhotoCurrent)) {
			...
			ContactsAsyncHelper.updateImageViewWithContactPhotoAsync(info, 0,
					this, call, getContext(), mPhoto, personUri, -1);
		}
	 ...
	 * needs a special photo (like "busy" or "dialing".)
	 * 
	 * If the current call does not require a special image in the "photo" slot
	 * onscreen, don't do anything, since presumably the photo image has already
	 * been set (to the photo of the person we're talking, or the generic
	 * "picture_unknown" image, or the "conference call" image.)
	 */
	private void updatePhotoForCallState(Call call) {
		if (DBG)
			log("updatePhotoForCallState(" + call + ")...");
		...
		default:
			...
			// If no photoResource found, check to see if this is a conference
			// call. If
			// it is not a conference call:
			// 1. Try to show the cached image
			// 2. If the image is not cached, check to see if a load request has
			// been
			// made already.
			// 3. If the load request has not been made [DISPLAY_DEFAULT], start
			// the
			// request and note that it has started by updating photo state with
			// [DISPLAY_IMAGE].
			// Load requests started in (3) use a placeholder image of -1 to
			// hide the
			// image by default. Please refer to CallerInfoAsyncQuery.java for
			// cases
			// where CallerInfo.photoResource may be set.
			if (photoImageResource == 0) {
				{
					if (!showCachedImage(mPhoto, ci)
							&& (mPhotoTracker.getPhotoState() == ContactsAsyncHelper.ImageTracker.DISPLAY_DEFAULT)) {
						ContactsAsyncHelper
								.updateImageViewWithContactPhotoAsync(ci,
										getContext(), mPhoto,
										mPhotoTracker.getPhotoUri(), -1);
						mPhotoTracker
								.setPhotoState(ContactsAsyncHelper.ImageTracker.DISPLAY_IMAGE);
					}
				}
		...
		if (photoImageResource != 0) {
			if (DBG)
				log("- overrriding photo image: " + photoImageResource);
			showImage(mPhoto, photoImageResource);
	 ...
	 * 
	 * @return true if we were able to find the image in the cache, false
	 *         otherwise.
	 */
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		if (mSlidingCardManager != null)
			mSlidingCardManager.handleCallCardTouchEvent(ev);
		return true;
	}
	...
	/**
	 * Returns the "Menu button hint" TextView (which is manipulated directly by
	 * the InCallScreen.)
	 * 
	...
	/**
	 * Updates anything about our View hierarchy or internal state that needs to
	 * be different in landscape mode.
	 * 
	 * @see InCallScreen.applyConfigurationToLayout()
	 */
	/* package */public void updateForLandscapeMode() {
		if (DBG)
			log("updateForLandscapeMode()...");
		...
		setSideMargins(mOtherCallOngoingInfoArea,
	...
	private void setSideMargins(ViewGroup vg, int margin) {
		ViewGroup.MarginLayoutParams lp = (ViewGroup.MarginLayoutParams) vg
				.getLayoutParams();
