...
public class CallScreen extends Activity implements
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(SPEAKER_MENU_ITEM)
				.setVisible(
						!(Receiver.headset > 0 || Receiver.docked > 0 || Receiver.bluetooth > 0));
		menu.findItem(ANSWER_MENU_ITEM).setVisible(
	...
	void disableKeyguard() {
		...
		if (enabled) {
			...
			if (Integer.parseInt(Build.VERSION.SDK) == 16
					&& Build.MODEL.contains("HTC One"))
				mKeyguardManager
						.exitKeyguardSecurely(new OnKeyguardExitResult() {
	...
	public void onResume() {
		...
		System.out.println(Receiver.call_state);
