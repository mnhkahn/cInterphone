...
public class Receiver extends BroadcastReceiver {
		...
		if (call_state != state) {
				...
				if ((pstn_state == null || pstn_state.equals("IDLE"))
					...
					if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {
						String sUriSipRingtone = PreferenceManager
								.getDefaultSharedPreferences(mContext)
								.getString(
										com.cyeam.cInterphone.ui.Settings.PREF_SIPRINGTONE,
			...
			case UserAgent.UA_STATE_HOLD:
				...
				if (InCallScreen.started
						&& (pstn_state == null || !pstn_state.equals("RINGING")))
					mContext.startActivity(createIntent(InCallScreen.class));
				break;
			}
			pos(true);
	...
	public static void onText(int type, String text, int mInCallResId, long base) {
		...
		if (type >= REGISTER_NOTIFICATION
				&& mInCallResId == R.drawable.presence_online
				&& !PreferenceManager.getDefaultSharedPreferences(
						Receiver.mContext).getBoolean(
						com.cyeam.cInterphone.ui.Settings.PREF_REGISTRATION,
						com.cyeam.cInterphone.ui.Settings.DEFAULT_REGISTRATION))
			text = null;
		...
		if (text != null) {
			...
			if (type == MISSED_CALL_NOTIFICATION) {
				...
				if (PreferenceManager.getDefaultSharedPreferences(
						Receiver.mContext).getBoolean(
						com.cyeam.cInterphone.ui.Settings.PREF_NOTIFY,
						com.cyeam.cInterphone.ui.Settings.DEFAULT_NOTIFY)) {
				...
				if (base != 0) {
					...
					if (PreferenceManager
							.getDefaultSharedPreferences(mContext)
							.getBoolean(
									com.cyeam.cInterphone.ui.Settings.PREF_POS,
									com.cyeam.cInterphone.ui.Settings.DEFAULT_POS))
	...
	static void updateAutoAnswer() {
		if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(
				...
				com.cyeam.cInterphone.ui.Settings.DEFAULT_AUTO_ONDEMAND)
	...
	public static void pos(boolean enable) {
		...
		if (!PreferenceManager.getDefaultSharedPreferences(mContext)
				.getBoolean(com.cyeam.cInterphone.ui.Settings.PREF_POS,
						com.cyeam.cInterphone.ui.Settings.DEFAULT_POS)
				|| PreferenceManager
						.getDefaultSharedPreferences(mContext)
						.getString(
								com.cyeam.cInterphone.ui.Settings.PREF_POSURL,
								com.cyeam.cInterphone.ui.Settings.DEFAULT_POSURL)
						.length() < 1) {
			if (lm != null && am != null) {
				pos_gps(false);
				pos_net(false);
		...
		if (enable) {
			if (call_state == UserAgent.UA_STATE_IDLE
				...
				if (System.currentTimeMillis() - loctrydate > GPS_UPDATES
						&& (last == null || System.currentTimeMillis()
								- last.getTime() > GPS_UPDATES)) {
					loctrydate = System.currentTimeMillis();
					pos_gps(true);
					pos_net(false);
				} else if (last != null)
					Receiver.url("lat=" + last.getLatitude() + "&lon="
							+ last.getLongitude() + "&rad="
							+ last.getAccuracy());
				pos_net(true);
			} else
				pos_net(false);
	...
	static void enable_wifi(boolean enable) {
		if (!PreferenceManager.getDefaultSharedPreferences(mContext)
				.getBoolean(com.cyeam.cInterphone.ui.Settings.PREF_OWNWIFI,
						com.cyeam.cInterphone.ui.Settings.DEFAULT_OWNWIFI))
			return;
		if (enable
				&& !PreferenceManager
						.getDefaultSharedPreferences(mContext)
						.getBoolean(
								com.cyeam.cInterphone.ui.Settings.PREF_WIFI_DISABLED,
								com.cyeam.cInterphone.ui.Settings.DEFAULT_WIFI_DISABLED))
			return;
		...
		edit.putBoolean(com.cyeam.cInterphone.ui.Settings.PREF_WIFI_DISABLED,
				!enable);
		edit.commit();
		/*
		 * if (enable) { Intent intent = new
	...
	static boolean on_vpn() {
		return PreferenceManager.getDefaultSharedPreferences(mContext)
				.getBoolean(com.cyeam.cInterphone.ui.Settings.PREF_ON_VPN,
						com.cyeam.cInterphone.ui.Settings.DEFAULT_ON_VPN);
	}
	...
	static boolean isFastGSM(int i) {
		...
		if (tm.getNetworkType() >= TelephonyManager.NETWORK_TYPE_UMTS)
			return PreferenceManager.getDefaultSharedPreferences(mContext)
					.getBoolean(
							com.cyeam.cInterphone.ui.Settings.PREF_3G
									+ (i != 0 ? i : ""),
							com.cyeam.cInterphone.ui.Settings.DEFAULT_3G);
		if (tm.getNetworkType() == TelephonyManager.NETWORK_TYPE_EDGE)
			return PreferenceManager.getDefaultSharedPreferences(mContext)
					.getBoolean(
							com.cyeam.cInterphone.ui.Settings.PREF_EDGE
									+ (i != 0 ? i : ""),
							com.cyeam.cInterphone.ui.Settings.DEFAULT_EDGE);
		return false;
	}
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intentAction.equals(Intent.ACTION_BOOT_COMPLETED)) {
			...
			if (PreferenceManager
					.getDefaultSharedPreferences(mContext)
					.getBoolean(
							com.cyeam.cInterphone.ui.Settings.PREF_SELECTWIFI,
							com.cyeam.cInterphone.ui.Settings.DEFAULT_SELECTWIFI))
			...
			if (SystemClock.uptimeMillis() > lastscan + 45000
				...
				if (activeSSID != null && activeSSID.length() == 0)
					activeSSID = null;
